...
public class Ants {
	 ...
	 */
	public Ants(int loadTime, int turnTime, int rows, int cols, int turns, int viewRadius2, int attackRadius2, int spawnRadius2) {
	...
	public void update(Ilk ilk, Tile tile) {
		if (map[tile.getRow()][tile.getCol()].equals(Ilk.FOOD) && ilk.equals(Ilk.LAND)) {
			List<Aim> allDirections = Arrays.asList(Aim.values());
			for (Aim direction : allDirections) {
				Tile to = MyBot.ants.getTile(tile, direction);
				if (MyBot.antPop.getAntAtRowCol(to.getRow(), to.getCol()) != null) {
					MyBot.antQueue++;
					Util.addToLog("Eating at: " + tile.getRow() + " " + tile.getCol());
					break;
				}
			}
		}
